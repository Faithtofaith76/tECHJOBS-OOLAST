...
import static org.launchcode.models.forms.JobForm.*;
...
public class JobController {
    ...
    public String add(Model model, @ModelAttribute @Valid JobForm jobForm, Errors haserrors, JobData jobData) {

        // TODO #6 - Validate the JobForm model, and if valid, create a
        // new Job and add it to the jobData data store. Then
        // redirect to the job detail view for the new Job.
        if  (jobForm.getName().length() == 0) {
            model.addAttribute("errors", haserrors);
            return "new-job";
        }
